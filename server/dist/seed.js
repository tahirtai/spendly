...
async function seedDatabase() {
    try {
        ...
        for (const name of categories) {
            await supabase_js_1.supabaseAdmin.from('ExpenseCategory').upsert({ name, isDefault: true }, { onConflict: 'name' });
        }
        ...
        const { data: authUsersData, error: listError } = await supabase_js_1.supabaseAdmin.auth.admin.listUsers();
        let adminId = null;
        if (!listError && authUsersData?.users) {
            ...
            if (existingAuthUser) {
                ...
                const { error: updateError } = await supabase_js_1.supabaseAdmin.auth.admin.updateUserById(adminId, {
                    email_confirm: true,
        ...
        if (!adminId) {
            ...
            const { data: newAuthData, error: createError } = await supabase_js_1.supabaseAdmin.auth.admin.createUser({
                email: adminEmail,
                password: adminPassword,
                email_confirm: true,
                user_metadata: { fullName: 'Spendly Super Admin', role: 'SUPER_ADMIN' }
        ...
        if (adminId) {
            // 1. App User record
            const { error: userTableError } = await supabase_js_1.supabaseAdmin.from('User').upsert({
                id: adminId,
                email: adminEmail,
            ...
            let { data: ws } = await supabase_js_1.supabaseAdmin
                ...
                .maybeSingle();
            if (!ws) {
                const { data: newWs } = await supabase_js_1.supabaseAdmin
                    .from('Workspace')
            ...
            if (ws) {
                ...
                const { data: existingPrice } = await supabase_js_1.supabaseAdmin
                    .from('MealPrice')
                    .select('*')
